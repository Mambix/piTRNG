...
char* SPIClass::Read(int size)
{
	...
	//Transfer many bytes
	char* data_buffer_array = this->ReadManyTimes(size);
...
char* SPIClass::ReadManyTimes(int size)
{	
	//Transfer many bytes
	char data_buffer[size];
	int Count;
	for (Count = 0; Count < size; Count++)
		data_buffer[Count] = 0x55;
    char* data_buffer_array = &data_buffer[0];
